        ...
        for server_address in server_addresses:
            try:
                with context.wrap_socket(
                    socket.socket(), server_hostname=server_address
                ) as s:
                    ...
                    cert = load_pem_x509_certificate(
                        cert_der.encode(), default_backend()
                    )
                    cert_store.append(cert)
                    context.verify_mode = ssl.CERT_REQUIRED
                    context.check_hostname = (
                        False  # Disable default hostname validation
                    )
                ...
                error_message = str(e)
                print(
                    f"Hostname validation failed for {server_address}: {error_message}"
                )
...
if __name__ == "__main__":
    ...
    if not server_addresses:
        ...
        server_addresses = input(
            "Enter the server addresses (separated by spaces): "
        ).split()
